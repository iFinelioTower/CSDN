...
static int do_offset = 0;
static int MAX_x = 32767;
static int MAX_y = 32767;
static char offset_flag=0;
ssize_t ret;

#define OFFSET_FILE "/data/private/cbstate"
		...
		unsigned char data, unsigned int flags)
{
	...
	case 0:
		if (touchit213->data[0]  != T213_FORMAT_STATUS_BYTE) {
	...
	case 1:
		if (touchit213->data[1] !=	T213_FORMAT_STATUS_MASK) {
			pr_debug("unsynchronized data: 0x%02x\n", data);
			touchit213->idx = 0;
		}
		break;
	...
	case 8:
		report_x = ((touchit213->data[4] << 8) | touchit213->data[3]) << 3;
		report_y = ((touchit213->data[6] << 8) | touchit213->data[5]) << 3;
	
		if(do_offset) {
			do_offset = 0;
			file = filp_open(OFFSET_FILE, O_RDONLY,0);
		
			if (IS_ERR(file)) {
				printk("open %s failed.", OFFSET_FILE);
				//return -ENOENT;
			} else {
				file->f_op->llseek(file, 0, 0);
				ret = file->f_op->read(file, &offset_flag, 1, &file->f_pos);
		...
		case 0:
			break;
		case 1:
			report_x = MAX_x - report_x;
			break;
		case 2:
			report_x = MAX_x - report_x;
			report_y = MAX_y - report_y;
			break;
		case 3:
			report_y = MAX_y - report_y;
			break;
		...
		/*
		if (touchit213->data[2] == 0x84)
			touch=0;
		else if(touchit213->data[2] == 0x82)
		  touch=1;
		*/
		touch = (touchit213->data[2] & 0x03) ? 1 : 0;

		printk("=====report_x:%d, report_y:%d\n", report_x, report_y);
		input_report_key(dev, BTN_TOUCH, touch);
		input_report_abs(dev, ABS_X, report_x);
		input_report_abs(dev, ABS_Y, report_y);
		input_sync(dev);
...
static int touchit213_connect(struct serio *serio, struct serio_driver *drv)
{
	...
	input_dev->name = "Sahara_It213_Serial_Touchscreen";
